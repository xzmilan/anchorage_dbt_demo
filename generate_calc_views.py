...
======================
Assembles individual micro-calc SQL files into unified BigQuery views or dbt models.

Mirrors the AO microtools CI/CD pattern:
  - Each subfolder of calcs/ is a calc group (e.g. CustodyPositionsCalc/)
  - Each .sql file inside is one metric (e.g. PositionPeriod.sql)
  - This script assembles all .sql files for a group into a single wide table
    where ID is the join key and each calc becomes one column.

Usage:
  python generate_calc_views.py                # execute combined views directly in BigQuery
  python generate_calc_views.py --dry-run      # print resolved SQL only, no BigQuery calls
  python generate_calc_views.py --generate-dbt # write combined dbt model .sql files to
                                               # models/Calc_Anchorage/ (Jinja refs preserved)

Example output for CustodyPositionsCalc:
  -- models/Calc_Anchorage/CustodyPositionsCalc.sql (dbt model)
...
CALC_ROOT = Path(__file__).parent / 'calcs'
DBT_MODELS_PATH = Path(__file__).parent / 'models' / 'Calc_Anchorage'
...
def build_dbt_model_sql(calc_dir: Path) -> str | None:
    """
    Given a calc group directory (e.g. calcs/CustodyPositionsCalc), return a
    dbt-compatible SELECT statement (Jinja source() refs preserved) that assembles
    all micro-calcs as columns in a single wide table.
    ...
    base_source = f"{{% source('anchorage_data_platform', '{base_table}') %}}"
    ...
    from_str = f'FROM {{{{ source(\'anchorage_data_platform\', \'{base_table}\') }}}} AS Base\n' + '\n'.join(join_lines)

    header = f'-- AUTO-GENERATED by generate_calc_views.py — do not edit by hand.\n-- Source calcs: calcs/{folder_name}/\n-- Regenerate: python generate_calc_views.py --generate-dbt\n'
    return f'{header}\n{with_clause}\n{select_str}\n{from_str}\n'


def build_view_sql(calc_dir: Path) -> str | None:
    """
    Given a calc group directory (e.g. calcs/CustodyPositionsCalc),
...
def main(dry_run: bool = False, generate_dbt: bool = False) -> None:
    ...
    if generate_dbt:
        ...
        for calc_dir in calc_dirs:
            model_sql = build_dbt_model_sql(calc_dir)
            if model_sql is None:
                print(f'  Skipping {calc_dir.name} — no .sql files found')
                continue
            out_path = DBT_MODELS_PATH / f'{calc_dir.name}.sql'
            out_path.write_text(model_sql)
            print(f'  ✓ Wrote {out_path.relative_to(Path(__file__).parent)}')
        print('\nDone. Run `dbt run --select Calc_Anchorage` to materialize.')
        return
...
if __name__ == '__main__':
    ...
    generate_dbt = '--generate-dbt' in sys.argv
    main(dry_run=dry_run, generate_dbt=generate_dbt)
